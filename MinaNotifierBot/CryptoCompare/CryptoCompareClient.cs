...
namespace MinaNotifierBot.CryptoCompare
{
	public class CryptoCompareClient
	{
        ...
        public MarketData GetMarketData()
        {
            string str =
                Download(
                    $"https://min-api.cryptocompare.com/data/price?fsym=MINA&tsyms=BTC,USD,EUR,ETH&api_key={_cryptoCompareToken}");
        ...
        public async Task<List<OHLC>> GetOHLC()
        {
            ...
            if (histData1?.Data?.Data == null)
            {
                logger.LogDebug($"histominute result ({histUrl}): " + histDataStr);
